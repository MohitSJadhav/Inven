...
export default function SearchUser() {
    ...
    const handleSubmit = async (event) => {
        ...
        if (searchValue === "" || searchFilterValue === "") {
            ...
            try {
                const queryresponse = await axios.get(`http://localhost:8081/api/v1/inventory/user/search?email=${searchValue}&filter=${searchFilterValue}`);
                    ...
                    <List divided={true}>
                        {response.data.map((value, index) => {
                            return <Segment padded='very'>
                                <ListItem key={index} >
                                    <Grid columns={6}>
                                        <GridColumn><ListContent><ListHeader ><h4>Email ID</h4></ListHeader>{value.email}</ListContent><br></br></GridColumn>
                                        <GridColumn><ListContent><ListHeader ><h4>User Name</h4></ListHeader>{value.username}</ListContent><br></br></GridColumn>
                                        <GridColumn><ListContent><ListHeader><h4>Group Name</h4></ListHeader>{value.groupname}</ListContent><br></br></GridColumn>
                                        <GridColumn><ListContent><ListHeader><h4>Status</h4></ListHeader>{value.status}</ListContent><br></br></GridColumn>
                                        <GridColumn><ListContent><ListHeader><h4>Update Date</h4></ListHeader>{value.updatedate}</ListContent><br></br></GridColumn>
                                    </Grid>
                                </ListItem>
                            </Segment>
                        })}
