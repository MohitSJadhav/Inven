...
export default function DeleteNetworkConfig() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            queryresponse = await axios.delete(`http://localhost:8081/api/v1/inventory/networkconfig/remove?hostid=${deleteValue}`);
        ...
        <>
            <Segment>
                <Form onSubmit={handleSubmit}>
                    ...
                    <FormButton primary type="submit" name="button">Delete Network Config</FormButton>
                </Form>
            </Segment>

            {isExceptionDeleted && renderErrorValue ?
                <Message positive>
                    <MessageHeader>Deleted Network Config with id: {deleteErrorValue}</MessageHeader>
                    <p>Good Job!</p>
                </Message> : <></>}
            {!isExceptionDeleted && renderErrorValue ?
                <Message negative>
                    <MessageHeader>Sorry, we couldn't locate an Network Config with id: {deleteErrorValue}</MessageHeader>
